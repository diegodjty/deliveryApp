...
const BascketScreen = () => {
  ...
  useMemo(() => {
    const groupedItems = items.reduce((results, item) => {
      (results[item.dish._id] = results[item.dish._id] || []).push(items);
      return results;
    }, {});
        ...
        <ScrollView className="divide-y divide-gray-200">
          {Object.entries(groupedItemsInBasket).map(([key, items]) => (
            ...
            >
              ...
              <Image
                source={{
                  uri: urlFor(items[0][0].dish?.image).url(),
                }}
                className="h-12 w-12 rounded-full"
              />
              <Text className="flex-1">{items[0][0]?.dish.name}</Text>
              <Text className="flex-1">${items[0][0]?.dish.price}</Text>
              <TouchableOpacity>
                <Text
